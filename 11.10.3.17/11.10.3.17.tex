...
\item Length of altitude
	...
	Distance between vertex $\vec{A}$ and any point on line BC is given by

	\begin{align}
		d &= \norm{\vec{A} - \brak{\vec{B}+ \mu \vec{m}}} \\
	\end{align}

	The length of altitude can be finding the minma of $d$.

	\begin{align}
		f(\mu) &=\norm{\vec{A} - \brak{\vec{B}+ \mu \vec{m}}}^2  \\
		&= \norm{\vec{A} - \vec{B}}^2 - 2 \mu \vec{m}^{\top} \brak{\vec{A} - \vec{B}} + \mu^2 \norm{\vec{m}}^2
	\end{align}
	...
	\begin{align}
		...
		\mu &= \frac{\vec{m}^{\top} \brak{\vec{A} - \vec{B}}}{\norm{\vec{m}}^2} \\
	...
	\begin{align}
		\frac{\partial^2 f}{\partial \mu^2} &= 2 \norm{\vec{m}}^2 > 0
	\end{align}
	
	so $\mu = -1$ is the minima.
...
Therefore, the length of alititude is $\sqrt{2}$
